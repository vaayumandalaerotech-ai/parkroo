...
STREETS_JSON_PATH = os.path.join(BASE_DIR, "melbourne_streets.json")
...
async def lifespan(app: FastAPI):
    ...
    # --- Load pre-compiled street names for fast autocomplete ---
    global common_street_names
    common_street_names = []
    try:
        with open(STREETS_JSON_PATH, 'r', encoding='utf-8') as f:
            common_street_names = json.load(f)
        logger.info(f"Successfully loaded {len(common_street_names)} common street names for autocomplete.")
    except (FileNotFoundError, json.JSONDecodeError) as e:
        logger.warning(f"Could not load common street names from {STREETS_JSON_PATH}: {e}")
...
@backend_app.get("/api/search-places")
def search_places(query: str):
    """
    Provides autocomplete suggestions for a given search query using Nominatim.
    """
    if not query or len(query) < 3:
        return []
    try:
        geolocator = Nominatim(user_agent="parking_finder_autocomplete")
        # Using exactly_one=False returns a list of possible locations
        locations = geolocator.geocode(query, exactly_one=False, limit=5, country_codes="AU")
        return [loc.address for loc in locations] if locations else []
    except Exception as e:
