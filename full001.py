        ...
        for item in api_records:
            ...
            if lat != 0 or lng != 0:
                processed_data.append({
                    ...
                    "fee": 3.0, "auTime": current_time.astimezone(au_tz).strftime("%H:%M %d/%m/%Y"), "inTime": current_time.strftime("%H:%M %d/%m/%Y")
                })
...
async def register_user(user: UserCreate, background_tasks: BackgroundTasks, db: sqlite3.Connection = Depends(get_db)):
    ...
    db.execute("INSERT INTO users (username, email, name, phone, hashed_password) VALUES (?, ?, ?, ?, ?)",
               (user.username, user.email, user.name, None, hashed_password))
    db.commit()
    if user.email:
...
def get_path(city: str, lat: float, lng: float, background_tasks: BackgroundTasks, top: int = 5):
    ...
    for spot in top_spots:
        try:
            route_data = _get_osrm_route(current[0], current[1], spot['lat'], spot['lng'], steps=False)
...
def get_route(start_lat: float, start_lon: float, end_lat: float, end_lon: float, steps: bool = False):
    try:
        return _get_osrm_route(start_lat, start_lon, end_lat, end_lon, steps=steps)
         ...
         raise HTTPException(status_code=503, detail=f"Routing service is unavailable or failed: {e}")
...
def run_frontend():
    ...
    def render_settings_view():
        ...
        if st.button("⬅️ Back to Map", key="exit_settings_btn", use_container_width=True):
    ...
    if st.session_state.get('show_reset_password_form', False):
        ...
        st.info("Please log in or register to access the map and parking features.")
        ...
        st.markdown("---")
        m = folium.Map(location=CITY_CENTERS["melbourne"], zoom_start=12, tiles="cartodbdark_matter" if st.session_state.dark_theme else "cartodbpositron")
        st_folium(m, width='100%', height=500)

    else:
        # --- LOGGED-IN STATE ---
        if not st.session_state.get('first_login_refresh_done', False) or not st.session_state.get('spots'):
            render_loading_screen(f"Finding parking spots in {st.session_state.selected_city.title()}...")
            ...
            if final_response and final_response.status_code == 200:
                ...
                if st.session_state.spots:
                    ...
                    st.warning("Could not load parking data. Please try refreshing.")
            else:
        ...
        if not st.session_state.get('spots'):
            render_loading_screen("Preparing your map...")
            st.stop()
        ...
        with st.container():
            ...
            with top_bar_cols[1]:
                destination_address = st.text_input("Enter Destination Address", placeholder="Search for destination or spot address", label_visibility="collapsed")
            
            with top_bar_cols[2]:
                st.button("Set Destination", key="set_dest_btn", use_container_width=True)
            
            with top_bar_cols[3]:
                n_cols = st.columns([1, 2])
                if not st.session_state.path_data:
                    with n_cols[0]:
                        top_n = st.number_input("N", min_value=1, max_value=10, value=3, step=1, label_visibility="collapsed")
                    with n_cols[1]:
                        if st.button(f"Find Spots", key="find_spots_btn", use_container_width=True):
                            handle_find_spots(top_n)
                else:
                    if st.button("Exit Path View", key="exit_path_view_btn", use_container_width=True):
            ...
            with top_bar_cols[5]:
                ...
                with menu_cols[1]:
                    if st.button("👑", key="view_admin_btn", help="Admin Dashboard", use_container_width=True):
                        ...
                        st.rerun()
                with menu_cols[2]:
                    if st.button("⚙️", key="view_settings_btn", help="Profile & Settings", use_container_width=True):
                        st.session_state.viewing_settings = True
                        st.session_state.viewing_bookings = False
                        st.rerun()

            if st.session_state.get('set_dest_btn'):
                ...
                del st.session_state.set_dest_btn
                st.rerun()
        ...
        if st.session_state.navigation_active:
            ...
            map_target = nav_cols[1]
        
        elif st.session_state.viewing_bookings:
            book_cols = st.columns([1, 3])
            with book_cols[0]:
                render_bookings_view()
            map_target = book_cols[1]

        elif st.session_state.viewing_settings:
            settings_cols = st.columns([1, 3])
            with settings_cols[0]:
                render_settings_view()
            map_target = settings_cols[1]
        
        elif st.session_state.viewing_admin:
            admin_cols = st.columns([1, 3])
            with admin_cols[0]:
                render_admin_view()
            map_target = admin_cols[1]
        ...
        with map_target:
            ...
            folium.TileLayer('https://{s}.basemaps.cartocdn.com/light_only_labels/{z}/{x}/{y}{r}.png', attr='CartoDB', name='Hybrid View (Labels)', overlay=True).add_to(m)
            folium.TileLayer('https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png', attr='OpenTopoMap', name='Terrain View').add_to(m)
            ...
            with button_cols[1]:
                if st.session_state.selected_spot and not st.session_state.selected_spot.get('occupied'):
                    if not st.session_state.stripe_pk:
                        ...
                        if pk_response and pk_response.status_code == 200:
                    ...
                    if st.session_state.stripe_pk:
                        ...
                        st.link_button(f"Pay ${spot_details.get('fee', 2.5):.2f} and Book", url=payment_url, use_container_width=True)
                    else:
                        st.error("Stripe is not configured on the backend.")
        ...
        google_maps_css = """
        <style>
        .st-emotion-cache-18e3th9 { padding-top: 1rem; padding-bottom: 1rem; padding-left: 5%; padding-right: 5%; }
        .st-emotion-cache-1d3z3vf { padding-top: 1rem; }
        ...
        st.markdown("""
            <style>
            .leaflet-marker-icon.green, .leaflet-marker-icon.red { animation: pulse 1.5s infinite; }
