...
async def send_confirmation_email(email_to: str, username: str):
    ...
    <p>Happy parking!</p>
    """
    message = MessageSchema(subject="Welcome to Express Parking Australia!", recipients=[email_to], body=html, subtype="html")
    fm = FastMail(conf)
...
async def send_password_reset_email(email_to: str, username: str, token: str):
    ...
    message = MessageSchema(
        subject="Password Reset Request", recipients=[email_to], body=html, subtype="html"
    )
    fm = FastMail(conf)
    await fm.send_message(message)
...
def run_frontend():
    ...
    if st.session_state.get('show_reset_password_form', False):
        ...
        with st.container():
            ...
            with top_bar_cols[1]: # Destination Search Bar
                if not st.session_state.dest_lat:
            ...
            with top_bar_cols[2]: # Destination Buttons
                if not st.session_state.dest_lat:
            ...
            with top_bar_cols[5]:
                ...
                with menu_cols[1]:
                    if st.button("👑", key="view_admin_btn", help="Admin Dashboard", use_container_width=True):
                        ...
                        st.rerun()
                with menu_cols[2]:
                    # This button now toggles the settings view
        ...
        with map_target:
            ...
            if st.session_state.spots:
                ...
                if total_spots > 0:
                    ...
                    col1.metric("Available", f"{available_spots}")
                    col2.metric("Occupied", f"{occupied_spots}")
                    col3.metric("Total", f"{total_spots}")

                    # Display a visual progress bar for availability
                    st.progress(available_percent, text=f"{available_percent:.0%} Available")
            ...
            # --- Dark Mode Fix ---
            tile_provider = "cartodbdark_matter" if st.session_state.dark_theme else "cartodbpositron"
            m = folium.Map(location=map_center, zoom_start=14, tiles=tile_provider, attr="CartoDB")
