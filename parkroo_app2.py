...
<head>
    ...
    <style>
        * {
        ...
        .sidebar:not(.collapsed) + .map-container .sidebar-toggle { left: 350px; }
    ...
    <div class="app-container">
        <header class="header">
            ...
            <div class="header-actions">
                ...
                <button class="btn btn-primary" onclick="showModal('login')">Login / Sign Up</button>
            </div>
        </header>
    ...
    <script>
        ...
        function initMap() {
            try {
                ...
                // --- MODIFIED: Two-stage data loading ---
                // 1. Load local data immediately
                loadParkingData('temp', false); // Don't recenter map on this initial load

                // 2. Load live data in the background
                setTimeout(() => {
                    const selectedCity = document.getElementById('cityFilter').value;
                    loadParkingData(selectedCity, true); // Recenter map after live data loads
        ...
        async function loadParkingData(city, isLiveData = true) {
        ...
        function showModal(type) {
            ...
            if (type === 'login') {
                ...
                body.innerHTML = `
                    <p>Login functionality is coming soon!</p>
                    `;
                ...
                body.innerHTML = `
                    <div style="display: flex; align-items: center; gap: 20px;">
                        ...
                        <div>
                            <h3 style="margin: 0; color: var(--text-primary);">Guest User</h3>
                            <p style="color: var(--text-secondary); margin: 0;">guest@parkroo.com</p>
                        </div>
                    </div>
                    <hr style="margin: 20px 0; border: none; border-top: 1px solid var(--border-color);">
                    <div>
                        <h4>My Vehicles</h4>
                        <p style="margin: 5px 0;"><strong>Honda Civic</strong> - ABC 123</p>
                        <p style="margin: 5px 0;"><strong>Toyota Camry</strong> - XYZ 789</p>
                    </div>
